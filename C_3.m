...
%                      |                       |     
F  = @(v)[10*(v(1)-(1/3)*((v(1))^3)-v(2)), v(1)/10]'
%     			 
...
yInitial    = [0.5,0.5]';                  % Initial value of y (2 x 1 column vector) 
E_glob_tol  = 0.0005;                     % Global error tolerance 
...
while t(i) < tFinal
    ...
    Err_h_temp = abs(y_hold(1,1) - y_half_step(1, 1))/3;
    ...
    while E_glob_est > (E_glob_tol/(tFinal - tInitial))
        ...
        h_temp_hold = h_temp;
        h_temp = 0.90 * h_temp * sqrt((E_glob_tol/10)*(h_temp/Err_h_temp));
    ...
    while E_glob_est < 0.8*(E_glob_tol/(tFinal - tInitial))
        ...
        h_temp_hold = h_temp;
        h_temp = 1.10 * h_temp * sqrt((E_glob_tol/10)*(h_temp/Err_h_temp));
...
% plot solution, error and global error estimate
figure(1)
plot(t,y(1,:));
